...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        t1 = time()
        self.R_train = sp.dok_matrix((self.n_users, self.n_items),
                                     dtype=np.float32)
        self.R_test = sp.dok_matrix((self.n_users, self.n_items),
                                    dtype=np.float32)
        ...
        print('Complete. Interaction matrices R_train and R_test created in',
              time() - t1, 'sec')
    ...
    def create_adj_mat(self):
        ...
        print('Complete. Adjacency-matrix created in', adj_mat.shape,
              time() - t1, 'sec.')
        ...
        print(
            'Complete. Transformed adjacency-matrix to NGCF-adjacency matrix in',
            time() - t2, 'sec.')
    ...
    def negative_pool(self):
        ...
        for u in self.train_items.keys():
            neg_items = list(
                set(range(self.n_items)) - set(self.train_items[u]))
...
data = Data(path='./data', batch_size=24)
